...
5. **Сохранение графика в файл/галерею**  
   В текущей версии реализация сохранения проста, поскольку это тестовое задание, а не реальный проект.  
   - **Как можно улучшить**:  
     - Перенести процесс сохранения в `Service`, чтобы приложение повысить приоритет приложению и для продолжения скачивания, например, после перезапуска устройства (`START_STICKY`).  
     - Показывать уведомления (push-нотификации) о ходе и результате скачивания — например, с индикатором прогресса и статусом «сохранено».  
   - **Почему не реализовано так сейчас**:  
     - Настройка фоновых сервисов, работа с push-нотификациями и runtime–permission-ом распространялась через несколько версий Android, постоянно меняясь. Чтобы корректно обработать все кейсы (разные API-уровни, поведение с ограничениями на фоновую работу, Scoped Storage, новые требования к уведомлениям), потребовалось бы значительное количество времени, что выходит за рамки тестового задания.

6. **Рекомендация по Dagger 2**  
   Для более масштабируемого и поддерживаемого DI в продакшн-проекте рекомендуется заводить отдельный скоуп для каждой фичи или модуля:  
   - Также не сделано для экономии времени
